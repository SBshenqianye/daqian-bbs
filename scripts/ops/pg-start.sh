...
#
# 数据安全（2026-08-05 修复）:
#   * 使用具名卷 bbs-pgdata 持久化数据 —— 容器删除/重建不丢数据
#   * 端口映射缺失时只重启容器，不再"删容器重建"（旧逻辑会换新卷丢数据）
# ============================================
set -e
...
create_container() {
    info "创建 PostgreSQL 容器（数据卷: $PG_VOLUME）..."
    $RUNNER run -d \
        --name "$PG_CONTAINER" \
        ...
        -e POSTGRES_DB="bbs" \
        -p ${PG_PORT}:5432 \
        -v ${PG_VOLUME}:${PG_DATA_DIR}:Z \
        "$PG_IMAGE"
    ok "容器已创建，数据卷 ${PG_VOLUME} 挂载 ${PG_DATA_DIR}"
}
...
case "$ACTION" in
    start)
        if $RUNNER container exists "$PG_CONTAINER" 2>/dev/null; then
            if $RUNNER ps --filter "name=$PG_CONTAINER" --filter "status=running" | grep -q "$PG_CONTAINER"; then
                ...
                if ! check_port_mapping; then
                    warn "端口映射 5432->$PG_PORT 缺失，尝试重启容器恢复..."
                    $RUNNER restart "$PG_CONTAINER"
                    sleep 3
                    if ! check_port_mapping; then
                        warn "重启后端口映射仍缺失，请手动检查: $RUNNER port $PG_CONTAINER"
                    fi
                fi
            else
        ...
        for i in $(seq 1 15); do
            if $RUNNER exec "$PG_CONTAINER" pg_isready -U work_flow >/dev/null 2>&1; then
                ok "PostgreSQL 就绪"
                break
            fi
            sleep 2
    ...
    status)
        if $RUNNER ps --filter "name=$PG_CONTAINER" --filter "status=running" | grep -q "$PG_CONTAINER" 2>/dev/null; then
            ok "PostgreSQL 运行中"
            $RUNNER exec "$PG_CONTAINER" psql -U work_flow -c "SELECT version();" 2>/dev/null
